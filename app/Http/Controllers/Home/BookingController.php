...
class BookingController extends HomeController
{
    ...
    public function bedCount($cityId, $expectedRoomNumber, $leastBed)
    {
        ...
        foreach ($city->hotels as $hotel) {
            $beds = [];
            $quantities = $hotel->roomQuantities;
    ...
    {
        $ratio1 = $this->currency->getRatio($firstId);
        $ratio2 = $this->currency->getRatio($secondId);
    ...
    public function booking(BookingRequest $request)
    {
        ...
        $period = date_diff(date_create($checkIn), date_create($checkOut));
        ...
        foreach ($hotel->roomQuantities as $quantity) {
            if (null != ($request->get(config('default.rq') . $quantity->id))) {
                $this->setBookingDetail($quantity->id, $request->get('rq-' . $quantity->id), $cardId));
            }
        }
